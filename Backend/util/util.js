...
//     return 1;
//   }
//   return parseInt(Math.log(n) / Math.log(256) + 1);
// }
...
function bigIntToBuffer(bn, byteSize = null) {
  if (byteSize === null) {
    ...
    // console.log(buf);
    // Revert to Number back for example block no. 305
    // parseInt("0x" + Buffer.from((305).toString(16)).toString())
    // const buf = Buffer.from(bn.toString(16));
    // console.log(buf);

    return buf;
  } else {
    let zeroPadding = byteSize - bytesNeeded(bn);
    // if (zeroPadding > 0 && (zeroPadding + bytesNeeded(bn)) % 2) {
    //   zeroPadding = "0" + "0".repeat(zeroPadding * 2);
    // } else {
    //   zeroPadding = "0".repeat(zeroPadding * 2);
    // }
    if (zeroPadding > 0) {
      zeroPadding = "0".repeat(zeroPadding * 2);
    }

    if ((zeroPadding + bn.toString(16).length) % 2) {
      zeroPadding = zeroPadding + "0";
    }

    const temp = Buffer.from(zeroPadding + bn.toString(16), "hex");
    // console.log(BigInt("0x" + temp.toString("hex")));
    return temp;
  }
}
